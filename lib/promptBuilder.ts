...
## 내담자 정보
이름: ${client.name || "(비공개)"} / 성별: ${client.gender || "미상"} / 연령: 만 ${client.age || "미상"}세 / 분석일: ${new Date().toLocaleDateString("ko-KR")}
...
## 출력 형식
다른 설명이나 마크다운 없이, 아래 JSON 스키마와 정확히 동일한 키 구조를 가진 순수 JSON 객체만 반환하십시오. 실시하지 않은 검사와 관련된 키는 절대 포함하지 마십시오. 각 텍스트 값 안에도 "**굵게**", "*기울임*", "#제목", "-불릿" 같은 마크다운 문법을 절대 쓰지 말고 순수한 문장으로만 작성하십시오.
