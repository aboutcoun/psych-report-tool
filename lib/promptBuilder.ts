...
export type PromptTarget = "client" | "counselor";
...
export function buildPrompt(body: ReportRequestBody, target: PromptTarget): string {
...
- 성격척도: ${formatScores(tci.character)}`);
  ...
  dataBlocks.push(`## SCT (문장완성검사) 응답${target === "counselor" && sct.enabled ? " (영역별로 그룹핑됨)" : ""}\n${target === "counselor" ? formatSctByDomain(sct) : formatSct(sct)}`);
...
MMPI 임상척도와 재구성임상척도(RC)를 근거로 핵심 가설(내담자가 겪고 있을 것으로 보이는 심리적 어려움의 핵심 주제)을 수립하십시오. 이 가설을, 내용척도·보충척도${tci.enabled ? "·TCI 결과" : ""}·SCT 응답을 참고하여 뒷받침되는지 검토하십시오. 여러 자료로 뒷받침되지 않거나 근거가 약한 가설은 보고서에 포함하지 말거나, 조심스러운 표현으로 제한적으로 제시하십시오.`);
  ...
  if (target === "client") {
    instructionBlocks.push(`**${stepNum++}) 작성 원칙 (이 보고서는 내담자 본인이 직접 읽습니다)**
...
- 종합 소견에서는 상담을 받는다면 어떤 도움을 받을 수 있을지 다양한 방식으로 폭넓게 제시하고, 특정 상담 기법(예: 인지행동치료) 하나만 반복해서 언급하지 마십시오.`);
    ...
    instructionBlocks.push(`**${stepNum++}) 작성 원칙 (이 보고서는 담당 상담자가 읽습니다)**
- 실제 척도명과 점수${mmpi.enabled ? ", 2-point code, RC/PSY-5/내용척도/보충척도 상승 패턴" : ""}을 구체적으로 인용하며 전문적으로 서술하십시오.
- 임상적 가설과 그 근거, 감별이 필요한 지점, 추가로 확인하면 좋을 부분을 포함하십시오.
- 상세하고 근거 중심적으로 작성하십시오.
- "counselor_notes"는 상담자가 실제 상담 장면에서 바로 참고할 수 있도록, 하나의 긴 문단이 아니라 **5~7개의 개별 포인트로 나누어 개조식으로** 작성하십시오. 각 포인트는 {"title": 짧은 소제목, "detail": 그 포인트에 대한 풍성하고 구체적인 설명(실제 개입 방법이나 접근 방식, 필요하면 구체적 문구 예시 포함, 3~5문장)} 형태입니다. 다뤄야 할 주제 예시: 라포 형성 시 유의점, 예상되는 저항/방어 패턴, 상담 초기 우선순위, 중기 상담 전략, 치료적 관계에서 주의할 점, (SCT를 실시한 경우) 위험 신호 유무와 대응. 각 포인트의 detail은 짧게 요약만 하지 말고 실제 상담 계획에 바로 쓸 수 있을 만큼 구체적으로 풀어서 작성하십시오.`);
    ...
    if (sct.enabled) {
      instructionBlocks.push(`**${stepNum++}) SCT 영역별 해석**
위에 영역별로 그룹핑된 SCT 응답을 검토하여, 각 영역별로 짧은 해석을 작성하십시오. 단, **모든 영역에 억지로 해석을 만들어내지 마십시오** — 그 영역의 응답에서 실제로 눈에 띄는 태도, 감정, 패턴이 발견될 때만 해당 영역을 포함하고, 특별히 주목할 점이 없거나 응답이 형식적/무난한 영역은 아예 결과에서 제외하십시오. 포함하는 경우 각 해석은 2~3문장으로 간결하게, 그 영역의 구체적인 응답 내용을 근거로 작성하십시오.`);
    }
  }
  ...
  partLines.push("- 종합 소견: 실시한 검사들을 아우르는 종합적 이해와 상담 방향 제안");

  const fields: string[] = [];
  if (target === "client") {
    if (mmpi.enabled) fields.push(`"validity_summary": "타당도 판정 결과, 쉬운 말로 (220자 내외, 무효 프로파일이면 그 사실을 분명히 명시)"`);
    if (tci.enabled) {
      fields.push(`"maturity_summary": "성격적 성숙도 (500자 내외)"`);
      fields.push(`"temperament_character_summary": "기질/성격적 특성 (600자 내외)"`);
    }
    if (mmpi.enabled) fields.push(`"symptom_summary": "증상/심리적 고통 (650자 내외)"`);
    fields.push(`"integration_recommendations": "종합 소견 (600자 내외)"`);
  } else {
    if (mmpi.enabled) fields.push(`"validity_summary": "타당도척도 수치 및 판정 근거 (300자 내외)"`);
    if (tci.enabled) {
      fields.push(`"maturity_summary": "성격적 성숙도의 임상적 근거와 함의 (550자 내외)"`);
      fields.push(`"temperament_character_summary": "TCI 기반 기질/성격 분석 (650자 내외)"`);
    }
    if (mmpi.enabled) fields.push(`"symptom_summary": "핵심가설-검증 구조에 따른 증상 분석 (750자 내외)"`);
    fields.push(`"integration_recommendations": "통합 해석 및 감별 포인트 (650자 내외)"`);
    fields.push(`"counselor_notes": [{"title": "포인트 소제목", "detail": "상세 설명 (150자 내외)"}, "... 총 5~7개"]`);
    if (sct.enabled) {
      fields.push(`"sct_domain_notes": [{"domain": "영역명 (가족/아버지/어머니/남자/여자/성, 이성, 결혼/과거/미래/대인관계/권위자/자아상/두려움/기타 중 실제 특이사항이 있는 것만)", "note": "그 영역에 대한 간결한 해석 (2~3문장)"}, "... 특이사항이 있는 영역만 포함, 없으면 이 필드 자체를 빈 배열로"]`);
    }
  }

  const roleLine =
    target === "client"
      ? `내담자 본인이 읽고 스스로를 이해하도록 돕는 "내담자용" 보고서를 작성하십시오.`
      : `담당 상담자가 상담 계획을 세우는 데 참고하는 "상담자용" 전문가 보고서를 작성하십시오.`;

  return `당신은 15년 이상의 임상 경험을 가진 임상심리전문가입니다. 아래 심리검사 결과(실시한 검사: ${testsAdministered.join(", ")})를 통합 분석하여, ${roleLine}
...
## 출력 형식
다른 설명이나 마크다운 없이, 아래 JSON 스키마와 정확히 동일한 키를 가진 순수 JSON 객체 하나만 반환하십시오. 실시하지 않은 검사와 관련된 키는 절대 포함하지 마십시오.
