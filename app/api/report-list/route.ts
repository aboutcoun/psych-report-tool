...
import { redis, REPORT_INDEX_KEY } from "@/lib/kv";
...
export async function GET() {
  try {
    ...
    const records: SavedReport[] = [];
    const staleKeys: string[] = [];

    for (const key of keys) {
      const record = await redis.get<SavedReport>(key);
      if (record) {
        records.push(record);
      } else {
        staleKeys.push(key); // TTL로 이미 삭제된 항목은 색인에서도 정리
      }
    }

    if (staleKeys.length > 0) {
      await redis.zrem(REPORT_INDEX_KEY, ...staleKeys);
    }
